...
function [R,T,Etotal,I_incoh_p,I_incoh_m] = Tmat2(lambda,n,L,theta_in)
[Rs,Ts,Eps,Ems,I_is_p,I_is_m] = Tmats(lambda,n,L,theta_in);
[Rp,Tp,Epp,Emp,I_ip_p,I_ip_m] = Tmatp(lambda,n,L,theta_in);
R=Rs/2+Rp/2;
T=Ts/2+Tp/2;
Es=abs(Eps+Ems).^2;
Ep=abs(Epp+Emp).^2;
...
Etotal=Es/4+Ep/4;
I_incoh_p=I_is_p/2+I_ip_p/2;
I_incoh_m=I_is_m/2+I_ip_m/2;
...
function [R,T,Epz,Emz,I_incoh_p,I_incoh_m] = Tmats(lambda,n,L,theta_in)
...
n_first=real(n(1)*cos(theta(1)));
n_Si=real(n(length(n)-1)*cos(theta(length(theta)-1)));
n_final=real(n(length(n))*cos(theta(length(theta))));
...
if(Incohtest1) %incoherent layer at the front
    ...
    mat1=1/abs(tAG1)^2*1/abs(phi1)^2*[1 -abs(rAG1)^2;abs(rAG1)^2 abs(tAG1*tGA1)^2-abs(rAG1*rGA1)^2]*[1 0;0 abs(phi1^2)^2];
...
if(Incohtest2) %incoherent layer at the back 
    ...
    mat2=1/abs(tGA2)^2*1/abs(phi2)^2*[1 0;0 abs(phi2^2)^2]*[1 -abs(rAG2)^2;abs(rAG2)^2 abs(tAG2*tGA2)^2-abs(rAG2*rGA2)^2];
    theta(length(n))=[];
    n(length(n))=[];
    L(length(L))=[];  
end
...
matmain=1/abs(tcd)^2*[1 -abs(rdc)^2;abs(rcd)^2 abs(tcd*tdc)^2-abs(rcd*rdc)^2];
if(Incohtest1&&Incohtest2)
    matfull=mat1*matmain*mat2;
    z=matfull(1,1);
    R=matfull(2,1)/z;
    T=1/z*n_final/n_first;
    MM=matmain*mat2;
    Efact=phi1*sqrt(matfull(1,1)*T/n_final*n_first);
    I_incoh_p=mat2(1,1)*T/n_final*n_Si;
    I_incoh_m=mat2(2,1)*T/n_final*n_Si;
    Epz=Epz*Efact;
    Emz=Emz*Efact;
elseif(Incohtest2)
...
function [R,T,Epz,Emz,I_incoh_p,I_incoh_m] = Tmatp(lambda,n,L,theta_in)
% layers are assumed (somewhat arbitrarily) to become incoherent at +0.1mm
Incohtest1=logical(L(1)>=100000);
Incohtest2=logical(L(length(L))>=100000);
...
for i=1:lay
     ...
     for z=Lsum(i):Lsum(i+1)-1
         Epz(z+1)=Ep(i)*exp(1i*k(i+1)*(z-Lsum(i)));           %again pay attention to index of k, we need the k in the layer                                                     
         Emz(z+1)=Em(i)*exp(-1i*k(i+1)*(z-Lsum(i)));
     end
end
